// Clean static-only sbdb client implementation
...
export async function fetchSmallBodyForSim(
  ...
  const row = STATIC_ASTEROID_ROWS.find(
    (r) => String(r.spkid) === id || r.full_name.trim().startsWith(id + " ")
  );
  if (!row) return null;
...
function normalizeStaticRow(r: any): SmallBodyForSim | null {
  ...
  const diameter_m =
    r.diameter_km != null ? Number(r.diameter_km) * 1000 : undefined;
  return {
    id: String(r.spkid),
...
export async function fetchCuratedForSim(
  ids?: string[]
): Promise<SmallBodyForSim[]> {
  if (ids && ids.length) {
    const out: SmallBodyForSim[] = [];
    for (const id of ids) {
      const row = await fetchSmallBodyForSim(id);
      if (row) out.push(row);
    }
...
export async function fetchBySpkidsForSim(
  _spkids: (string | number)[]
): Promise<SmallBodyForSim[]> {
  return [];
}
export async function fetchByDesignationsForSim(
  _des: string[]
): Promise<SmallBodyForSim[]> {
  return [];
}
