...
import { ImpactLocation } from "../data/simulation";
...
const SimulationGlobe: React.FC<SimulationGlobeProps> = ({
  onLocationSelect,
  impactLocation,
  damageZones = [],
}) => {
  ...
  useEffect(() => {
    ...
    if (cesiumContainerRef.current && !viewerRef.current) {
      ...
      if (viewer.scene && viewer.scene.globe) {
        ...
        viewer.scene.globe.atmosphereBrightnessShift = -0.33;
      }
    ...
    if (impactLocation) {
      ...
      viewer.camera.flyTo({
        destination: Cartesian3.fromDegrees(
          impactLocation.longitude,
          impactLocation.latitude,
          1000000 // 1000km altitude for good overview
        ),
        duration: 2.0,
      });
    }
    ...
    damageZones.forEach((zone, index) => {
      const entity = viewer.entities.add({
        ...
        ellipse: {
          ...
          height: 0,
          heightReference: HeightReference.CLAMP_TO_GROUND,
          zIndex: damageZones.length - index, // Smaller circles on top
        },
      });
        ...
        <div className="absolute bottom-4 left-4 bg-slate-900/90 backdrop-blur rounded-lg p-4 border border-blue-400/30 max-w-xs">
          ...
          <div className="text-white text-sm space-y-1">
            ...
            <div className="text-slate-300">
              Population:{" "}
              {formatPopulationDensity(impactLocation.populationDensity)} per
              km²
            </div>
          </div>
        </div>
...
function getPopulationDensityForLocation(
  ...
  const cities = [
    ...
    { name: "Los Angeles", lat: 34.0522, lon: -118.2437, density: 3198 },
    { name: "Mumbai", lat: 19.076, lon: 72.8777, density: 20482 },
    { name: "Beijing", lat: 39.9042, lon: 116.4074, density: 1311 },
  ];
  ...
  // Default density based on location
  if (Math.abs(lat) > 80 || Math.abs(lat) < 5) {
    return { density: 10 }; // Polar or ocean regions
  } else if (Math.abs(lat) < 30) {
    return { density: 500 }; // Tropical regions
  } else {
    return { density: 200 }; // Temperate regions
  }
}

function formatPopulationDensity(density: number): string {
  return new Intl.NumberFormat().format(Math.round(density));
}
...
export default SimulationGlobe;
